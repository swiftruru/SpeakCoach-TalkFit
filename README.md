...
- **完整示範流程**：點擊左側浮動導覽中的 `開始示範` 後，會先跑 10 秒 sample session，再完整導覽報告、首頁、歷史與設定
...
- **左側雙欄浮動導覽**：桌面版將控制列固定在手機左側，左欄放模式、控制與章節，右欄放當前步驟說明，不會把重點擠到最下方
- **開始示範 / 自由探索**：左側導覽可直接切換展示模式；`開始示範` 一按就啟動完整 demo，`自由探索` 則保留手動切頁
- **示範控制器**：示範中可直接在浮動導覽內執行上一步、下一步、暫停 / 繼續、倍速與結束示範
...
| `開始示範 / 自由探索（小螢幕）` | 小螢幕裝置仍可從頂部切換展示模式；桌機版改由左側浮動導覽操作 |
...
- 左側浮動導覽中的 `開始示範` 會先跑 10 秒 sample session，再自動帶到報告、首頁、歷史與設定；結束後會自動停止示範模式
