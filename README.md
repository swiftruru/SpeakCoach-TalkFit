...
| `⌘K 快速操作` | 開啟 Command Palette，快速跳頁或切換展示狀態 |
| `✦ Mock 資料` | 一鍵載入示範用練習紀錄與報告 |
| `收合說明 / 展開說明` | 開關右側說明面板，切換純展示與講解模式 |
| `輸出畫面` | 開啟 PNG 匯出面板，輸出作品集展示版、乾淨展示版或純手機版 |
| `簡報模式` | 進入更乾淨的展示版面，收起頂部工具列並改用精簡 HUD |
| `全螢幕` | 透過瀏覽器 `Fullscreen API` 進入真正全螢幕展示 |
