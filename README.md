...
- **首頁快速續練**：首頁會把最近一次練習整理成快速入口，可直接回看報告，或從上次最明顯的問題繼續重練
...
| 畫面 | 說明 |
|------|------|
| 首頁 | 顯示本週練習次數、平均贅字、每日趨勢、今日提醒，以及最近一次報告 / 問題片段的快速續練入口 |
| 練習中 | 顯示原型錄音狀態、語速儀表、練前確認卡、自動倒數、專注模式、分析中過場與片段重練提示 |
| 分析報告 | 顯示本次三重點、重練完成回饋、平均語速、語速曲線、問題片段跳轉、逐字稿標記、下一輪建議、片段一鍵重練與分享卡預覽 / 匯出 |
| 歷史紀錄 | 顯示累積練習統計、趨勢圖、每次練習列表與兩筆結果快速比較 |
...
- **說明面板只顯示當前項目**：右側只會列出目前畫面實際渲染出的說明項目，避免出現點了卻沒有箭頭可指向的卡片
...
- 右側說明面板只會列出目前畫面實際渲染出的說明項目，避免出現沒有對應箭頭的卡片
