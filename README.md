...
- **首頁任務導向補強**：首頁會根據最近一次練習結果整理「今天先做什麼」，可直接查看最近報告，或延續上次最主要的問題片段重新練習，不必再從歷史紀錄手動找入口
- **首頁統計卡版面穩定化**：本週練習、平均贅字與較上週三張摘要卡現在會保留一致的資訊區塊高度；即使某張卡沒有副文字，數值也會維持垂直置中，避免中英切換或內容長短不同時出現視覺偏移
- **介面收斂調整**：頂部 `展示工具` 已收斂成展示流程控制，保留說明面板、輸出、簡報模式、全螢幕與 `Quick Actions`；`雷射筆`、`游標強化` 則改留在簡報 HUD 與 `Command Palette`
- **簡報模式快捷退出**：進入簡報模式後可直接按 `Esc` 返回一般版面，投影展示時不必把滑鼠移回 HUD 點 `結束簡報`
- **投影輔助升級**：新增網站層 `雷射筆模式` 與 `游標強化模式`；雷射筆在移動時會帶紅色光束拖尾、停下來時只保留光點，游標強化則會放大觸控 ripple、箭頭與聚焦框，讓遠距離投影也更清楚
- **空閒提示**：桌面版閒置一小段時間後，右下角會輕提醒可按 `D`、`C`、`E`、`⌘K` 繼續操作，不需要一直記住網站層快捷鍵
- **示範聚焦同步補強**：完整示範播放時，分析報告會依步驟自動切換 `摘要 / 詳細`，設定頁也會依聚焦項目自動切到 `推薦 / 進階`，避免第 3–18 步因模式不符而出現沒框選、沒說明的情況
- **練習頁資訊位置微調**：即時逐字稿已往上移到目標進度卡後方，錄音中更早就能看到辨識內容，不會壓到接近底部導覽列的位置
...
- **語言與深連結**：網站外殼、模擬器內容、示範文案、說明卡與 Mock 資料都能即時切換繁中 / 英文，網址也會同步 `screen`、`panel`、`annotation`、`demo`、`step`、`theme`、`view`、`cursor`、`laser` 等展示狀態
...
- **首頁任務卡與快速續練**：首頁會把最近一次練習整理成「今天先做什麼」，可直接回看最近報告，或延續上次主要問題片段重新練習
...
- **雷射筆模式**：可切換成網站層紅色雷射筆，移動時帶光束拖尾、停下時保留光點，適合投影與作品集錄影時指出重點
- **游標強化模式**：可放大手機模擬器觸控點、ripple、聚焦框與註解箭頭，改善遠距離觀看時的辨識度
- **快速操作 Command Palette**：支援 `⌘K` / `Ctrl+K`，可快速跳頁、開始示範、切換簡報模式、進入全螢幕、開啟輸出畫面與執行重置
- **空閒提示卡**：網站閒置一段時間後會在右下角輕提醒常用快捷鍵，降低 demo 中斷時找不到下一步的成本
- **快捷鍵操作**：支援 `?`、`⌘K / Ctrl+K`、`D`、`P`、`F`、`E`、`A`、`X`、`C`、`R`、`T`、`S`、`L` 等桌面快捷鍵，提升作品集展示與面試 demo 效率
...
- **設計動機彈窗內語言切換**：開啟 `設計動機 / 關於` modal 後，也能直接在彈窗右上角切換繁中 / 英文，閱讀較長內容時不必先離開當前視窗
...
- **示範步驟同步切換**：demo 或註解聚焦到報告頁詳細區塊時，會同步切到 `詳細`；聚焦到設定頁進階項目時，也會自動展開 `進階設定`
...
- **手機版設計動機入口**：手機瀏覽時，底部右側會保留 `設計動機` 入口，方便在小螢幕直接查看產品背景與作者資訊
...
| 畫面 | 說明 |
|------|------|
| 首頁 | 顯示本週練習次數、平均贅字、每日趨勢、今日提醒，以及可直接回看最近報告或續練上次問題的「今天先做什麼」任務卡 |
...
- **示範模式同步切頁**：播放 demo 或從說明面板 pin 功能時，若目標位在報告頁詳細模式或設定頁進階模式，畫面會先同步切到正確模式再做聚焦
- **左右同步定位**：當右側說明卡驅動聚焦時，左側手機畫面會把對應區塊帶到較好閱讀的位置，不只停在邊緣
- **自動 Spotlight 聚焦**：桌面版 hover 或 demo 聚焦到功能區塊時，會自動壓暗其餘區域，只保留單一手機區塊與對應說明卡
- **錄音完成底部卡**：停止錄音後，會先以底部 sheet 整理這輪結果入口，讓使用者立刻決定是先看報告、再練一次，還是回首頁
- **即時逐字稿前移**：練習中的即時逐字稿已調整到目標進度卡後方，錄音時更容易在視線中段直接看到內容變化
- **簡報模式**：可切換成更乾淨的展示版面，收起頂部工具列，改用右上角精簡 HUD 操作
- **頂部按鈕分組**：`✦ 設計動機` 與 `✦ Mock 資料` 保持主層；其餘展示輔助操作收進 `展示工具` 與 `更多` 子選單，減少頂部工具列擁擠感
- **雷射筆模式**：可在簡報 HUD、`Command Palette` 或快捷鍵 `X` 切換網站層紅色雷射筆；移動時有光束、停下時保留光點
- **游標強化模式**：可在簡報 HUD、`Command Palette` 或快捷鍵 `C` 切換，放大觸控指示、hover 箭頭與 spotlight 邊線
- **空閒提示**：桌面版若停留一段時間沒操作，右下角會輕提醒可按 `D`、`C`、`E`、`⌘K`，幫助展示者快速接回節奏
...
- **全域鍵盤快捷鍵**：支援 `⌘K / Ctrl+K` 快速操作、`D` 開始 / 停止示範、`P` 切換簡報模式、`Esc` 結束簡報模式、`F` 全螢幕、`E` 輸出畫面、`A` 開關說明面板、`X` 雷射筆、`C` 游標強化、`R` 重置原型、`T` 切換主題、`S` 開啟設計動機、`L` 複製畫面連結
- **雙向 hover 導引**：滑鼠移到模擬器功能區塊時，箭頭會指向右側說明；移到說明卡時，箭頭則會反向指回模擬器
- **說明卡 pin / lock**：點擊說明卡可固定目前導引狀態，避免 hover 離開後立即跳掉
- **深連結分享**：網址現在會同步 `screen`、`panel`、`annotation`、`demo`、`step`、`theme`、`view`、`cursor`、`laser` 等展示狀態，適合直接分享精準畫面
...
| `展示工具` | 以子選單收納 `收合 / 展開說明`、`輸出畫面`、`簡報模式`、`全螢幕` 與 `⌘K 快速操作` |
| `開始示範（小螢幕）` | 小螢幕裝置可從頂部開始 / 停止示範；桌機版則改由左側浮動導覽操作 |
| `手機底部 設計動機` | 手機瀏覽時，底部右側提供 `設計動機` 入口，可直接打開產品介紹 / `關於` modal |
...
- `游標強化模式` 可用簡報 HUD、`Command Palette` 或 `C` 切換，會放大點擊 ripple、hover 箭頭與聚焦框，讓投影展示更清楚
- 若桌面版一段時間沒有操作，右下角會出現輕量 `空閒提示`，提醒可按 `D`、`C`、`E`、`⌘K`
...
- 手機版底部右側快捷入口已調整為 `設計動機`，用來快速開啟產品背景與作者資訊；行動版不再把註解面板當成主要補充資訊入口
- 模擬器內的觸控圓點與 ripple 現在固定維持在最上層，分析中 / 完成卡 / 啟動畫面等 overlay 出現時仍能清楚看出觸控位置
- `重置原型` 會清除所有練習資料與目前報告，不會自動重新塞回 mock 練習紀錄
- 畫面網址會同步 `screen`、`panel`、`annotation`、`demo`、`step`、`theme`、`view`、`cursor`、`laser` 等展示狀態，適合直接分享指定原型頁面給評審或面試官
