...
- **快捷鍵操作**：支援 `?`、`D`、`P`、`A`、`R`、`T`、`S`、`L` 等桌面快捷鍵，提升作品集展示與面試 demo 效率
...
- **全域鍵盤快捷鍵**：支援 `D` 開始 / 停止示範、`P` 切換簡報模式、`A` 開關說明面板、`R` 重置原型、`T` 切換主題、`S` 開啟設計動機、`L` 複製畫面連結
...
- `⌘ 快捷鍵` 視窗整理網站層操作；按 `?` 可直接開啟，方便在面試或 demo 前快速複習
