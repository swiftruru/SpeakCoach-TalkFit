...
| 分析報告 | 顯示平均語速、贅字排行、語速曲線、問題片段跳轉、逐字稿標記、分享卡預覽與匯出操作 |
...
- **展示層**：`PracticeScreen`、`ReportScreen`、`HistoryScreen`、`SettingsScreen` 各自訂閱對應 store 狀態，報告頁會再透過 marker helper 串起圖表與逐字稿定位，並掛上專用分享卡元件
...
├── lib/              # 分析邏輯、評分、贅字資料、情境設定、示範資料、分享卡匯出 helper
